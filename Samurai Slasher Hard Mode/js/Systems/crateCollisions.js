	...
	for(let id in entities) {
		...
		for(let id in entities) {
			...
			if(colliding(player, crate)) {
				...
				if(player.playerController.score > localStorage.getItem("samhighscore2")) {
					localStorage.setItem("samhighscore2", player.playerController.score);	
				}

				for(let id in entities)
					if(entities[id].has("text") && entities[id].has("score")) {
						entities[id].text.textObj.text = player.playerController.score;
					} else if(entities[id].has("text") && entities[id].has("samhighscore2")) {
						entities[id].text.textObj.text = "hi:" + localStorage.getItem("samhighscore2");
					}
