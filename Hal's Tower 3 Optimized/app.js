...
document.title = "Hal's Tower 3 Optimized - v" + VERSON;
...
function updateProjectiles(player, delta) {
    for(let i = 0; i < projectiles.length; ++i) {
        ...
        let row = map[Math.floor(projectiles[i].y / BLOCK_SIZE)];
        let block = row && row[Math.floor(projectiles[i].x / BLOCK_SIZE)];
...
function collidePlayerWithBlock(player, blockType, blockX, blockY) {
    ...
    if(blockType === MAP_BLOCK_TYPES.checkpoint) {
        if(collidingWithBlock(player, blockX, blockY, 0)) {
            const spawnChanged = player.spawnX !== blockX + BLOCK_SIZE / 4 ||
            ...
            // Persist immediately, but only when the checkpoint/gravity changes.
            if(spawnChanged) {
...
// The original timer ran at 250 Hz with six passes, five of which had
// almost zero delta. Keep its effective 4 ms physics step (8 px collision
// margins and fast projectiles need small steps), with one browser callback
// per display frame instead of 250 timer callbacks per second.
...
// Do not replay a background tab's physics when it becomes visible again.
// Timed effects still expire using wall-clock time, as in the original.
...
window.addEventListener('blur', () => { keys = []; touches = []; });
